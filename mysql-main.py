...
users = faker_generator.generate_users(50)
for user in users:
    insert_mysql.insert_user(cursor, user)
    vehicles = faker_generator.generate_vehicle(3, user['id'])
    for vehicle in vehicles:
        insert_mysql.insert_vehicle(cursor, vehicle)
        availabilities = faker_generator.generate_availability(2, vehicle['id'])
        for availability in availabilities:
            insert_mysql.insert_availability(cursor, availability)
        rentals = faker_generator.generate_rentals(2, user['id'], vehicle['id'])
        for rental in rentals:
            insert_mysql.insert_rental(cursor, rental)

companies = faker_generator.generate_sponsoring_companies(10)
for company in companies:
    ...
    for user_company in users_companies:
        insert_mysql.insert_user(cursor, user_company)
